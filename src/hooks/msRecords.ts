...
export const useMsRecords = (address?: string) => {
  const { records } = useRecords({
    filter: {
      programIds: [
        'wheres_alex_by_joey11_v1.aleo',
        'puzzle_pieces_by_joe11y_v1.aleo',
  ...
  const msGameRecords = records?.filter(
    (record) => record.programId === 'wheres_alex_by_joey11_v1.aleo'
  );
  const msPuzzleRecords = records?.filter(
    (record) => record.programId === 'puzzle_pieces_by_joe11y_v1.aleo'
  );
